...
jQuery.fn.passroids = function(o) { 
  ...
  var o = jQuery.extend( {
	  ...
	  button: null,
	  minimum: 0,
	  pwddontmatchtext: 'Passwords do not match',
	  pwdstrengthname: 'Strength',
	  seclevels: {0: 'Weak',1: 'Medium',2: 'Strong',3: 'Excelent' },
	  csslevels: {0: 'Weak',1: 'Medium',2: 'Strong',3: 'Excelent' },
	  showscore: true,
	  callback: false,
	  minpwlength : false,
	  pwdtooshortmsg : 'Too short'
  },o);
  ...
  return this.each(function() {
  	    // Check password strength ~~~~~~~~~~~~~~~~~~~~~
			...
			else { jQuery('#psr_score').html(''); return; }
	  		// Evaluate
	  		if(o.minpwlength && (val.length < o.minpwlength))
			{
				jQuery('#psr_score').html(o.pwdtooshortmsg);
				return;
			}
	  		var strength = evalScore(s);	
	  		// Display Score
			if( o.showscore === true )
				jQuery('#psr_score').html(o.pwdstrengthname+': <span class=psr_'+o.csslevels[strength]+'>'+o.seclevels[strength]+'</span>');
	    });
	...
	if(o.verify!=null){
		...
		jQuery(o.verify).blur(function(){
		   ...
		   if(main!=verify)
		   {
			   jQuery('#psr_verify').html(o.pwddontmatchtext);
			   if( o.callback != false )
				   o.callback();
