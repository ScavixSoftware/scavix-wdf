...
//        $info = "\$GLOBALS['translation']['properties'] = ".var_export($head,true);
	...
	function Translate($lang,$offset,$search,$untranslated)
	{
		...
		foreach( $rs as $term )
		{
            ...
            $cpy = Button::Textual('&raquo;')->addClass('copy')
                ...
                ;
            $btns = $cpy->WdfRender();
            if (($lang != $CONFIG['localization']['default_language'] ) && system_is_module_loaded('ai'))
            {
	...
	{
        return AjaxResponse::Json(['response' => ai_predict('Translate this text to '.strtoupper($lang).'. Please only return the translation and use formal "Sie":'.urldecode($text), [], 300)]);
	}
