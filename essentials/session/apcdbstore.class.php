...
/**
 */
class APCDBStore extends DbStore
{
    ...
    function Store(&$obj,$id="",$serialized_data=false)
    {
        ...
        $pid = pcntl_fork();
        if($pid === 0)
            parent::Store($obj, $id, $serialized_data);
    }
	...
	function Exists($id)
    {
		...
		if( isset($GLOBALS['object_storage'][$id]) )
            $res = true;
        elseif(apc_exists($GLOBALS["apcstore_key_prefix"].session_id().'_'.$id) === true)
            $res = true;
        else
...
//        log_debug('Migrate', $old_session_id, $new_session_id);
        ...
        $pid = pcntl_fork();
        if($pid === 0)
            parent::Migrate($old_session_id, $new_session_id);
