...
if (!isset($SAJAX_INCLUDED)) {
	...
	function sajax_handle_client_request() {
		...
		global $sajax_export_list, $CONFIG;
		...
		if (! in_array($func_name, $sajax_export_list))
			...
			echo "+:";
			$func_name = explode("__",$func_name);
			if( count($func_name) < 2 )
				$result = call_user_func_array($func_name[0], $args);
			else
			{
				$result = call_user_func_array(array($func_name[0], $func_name[1]), $args);
			}
	...
	function sajax_get_common_js() {
		...
		function sajax_do_call(func_name, args) {
			...
			if (sajax_request_type == "GET") {
				...
				if (uri.indexOf("?") == -1)
					uri += "?rs=" + saEscape(func_name);
				else
					uri += "&rs=" + saEscape(func_name);
				uri += "&rst=" + saEscape(sajax_target_id);
				uri += "&rsrnd=" + new Date().getTime();

				for (i = 0; i < args.length-1; i++)
					uri += "&rsargs[]=" + saEscape(args[i]);

				post_data = null;
			}
			else if (sajax_request_type == "POST") {
				post_data = "rs=" + saEscape(func_name);
				post_data += "&rst=" + saEscape(sajax_target_id);
				post_data += "&rsrnd=" + new Date().getTime();

				for (i = 0; i < args.length-1; i++)
					post_data = post_data + "&rsargs[]=" + saEscape(args[i]);
			}
			else {
