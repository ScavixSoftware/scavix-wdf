...
class Table extends Control
{
    ...
    {
        if( $this->footer )
            $this->_content = $this->prepend($this->footer);//array_merge(array($this->footer),$this->_content);
        if( $this->header )
            $this->_content = $this->prepend($this->header);//array_merge(array($this->header),$this->_content);

		if( $this->colgroup )
			$this->_content = $this->prepend($this->colgroup);//array_merge(array($this->colgroup),$this->_content);

        if( $this->Caption )
        {
			$this->_ensureCaptionObject();
            $this->_content = $this->prepend($this->Caption);//array_merge(array($this->Caption),$this->_content);
        }
